...
perm<-adonis2(dist.clr~coral.species*timepoint,as(sample_data(ps_qf),"data.frame"),by = "terms")
print(perm)
# coral species, time, and the interaction of species and time are significant at 0.05 level, but all the R2 values are very low (0.01626, 0.01444, 0.01568, respectively)

perm1<-adonis2(dist.clr~colony*timepoint,as(sample_data(ps_qf),"data.frame"),by = "terms")
print(perm1) 
# only the interaction of colony and time is significant at 0.05 level, R2 = 0.22632

perm1a<-adonis2(dist.clr~colony,as(sample_data(ps_qf),"data.frame"),by = "terms")
print(perm1a) # colony: not significant

perm1b<-adonis2(dist.clr~site*timepoint,as(sample_data(ps_qf),"data.frame"),by = "terms")
print(perm1b) # nothing significant

perm2<-pairwise.adonis2(dist.clr~site/coral.species,as(sample_data(ps_qf),"data.frame"), strata='coral.species', by="terms")
print(perm2)
# interaction of site and coral species significant only between LK and Pickles at 0.05 level

perm3<-pairwise.adonis2(dist.clr~timepoint/colony,as(sample_data(ps_qf),"data.frame"), strata='colony', by="terms")
print(perm3)
...
perm4<-pairwise.adonis2(dist.clr~site/coral.species,as(sample_data(ps_qf),"data.frame"), strata='coral.species', by="terms")
print(perm4) # site, controlling for coral species = not significant

perm5<-pairwise.adonis2(dist.clr~time/coral.species,as(sample_data(ps_qf),"data.frame"), strata='coral.species', by="terms")
...
# Create a bubble plot with select common or DA families in the top 25 - this is an abbreviated figure for presentation
...
unique(DAplot$Family) # 12 families
...
# [11] "Paracoccaceae"       "Phormidiaceae" 
